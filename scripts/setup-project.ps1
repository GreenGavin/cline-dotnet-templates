...
function Download-Template {
    param([string]$FileName, [string]$LocalPath)
    ...
    try {
        ...
        # Use proper PowerShell web request (FIXED)
        $response = Invoke-WebRequest -Uri $url -UseBasicParsing -ErrorAction Stop
        Set-Content -Path $LocalPath -Value $response.Content -Encoding UTF8
...
None at this time.
"@
    Set-Content -Path ".cline\context.md" -Value $contextContent -Encoding UTF8
...
"@
    Set-Content -Path ".cline\tasks.md" -Value $tasksContent -Encoding UTF8
...
"@
    Set-Content -Path ".cline\decisions.md" -Value $decisionsContent -Encoding UTF8
    Write-Host "✅ Created decisions.md" -ForegroundColor Green
}

# Download and save the current template version (FIXED)
try {
    Write-Host "📡 Checking template version..." -ForegroundColor Cyan
    $versionResponse = Invoke-WebRequest -Uri "$TemplateRepo/VERSION" -UseBasicParsing -ErrorAction Stop
    $version = $versionResponse.Content.Trim()
    Set-Content -Path ".cline\TEMPLATE_VERSION" -Value $version -Encoding UTF8
    Write-Host "✅ Template version: $version" -ForegroundColor Green
}
catch {
    Write-Warning "❌ Could not retrieve template version: $($_.Exception.Message)"
    Write-Host "📝 Setting default version to 1.0.0" -ForegroundColor Yellow
    Set-Content -Path ".cline\TEMPLATE_VERSION" -Value "1.0.0" -Encoding UTF8
}
